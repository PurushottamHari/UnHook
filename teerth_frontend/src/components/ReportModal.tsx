...
import { REPORT_REASONS } from '@/lib/services/article-interaction-service';
...
export default function ReportModal({
  ...
  const [selectedReasons, setSelectedReasons] = useState<string[]>(currentReasons);
  const [otherReason, setOtherReason] = useState<string>(currentOtherReason || '');

  useEffect(() => {
    if (isOpen) {
      setSelectedReasons(currentReasons);
      setOtherReason(currentOtherReason || '');
    }
  }, [isOpen, currentReasons, currentOtherReason]);
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 pointer-events-auto overflow-y-auto">
      ...
      <div className="flex items-center justify-center min-h-full w-full py-4 relative z-10">
        {/* Modal */}
        <div 
          className="relative bg-white dark:bg-amber-100 rounded-2xl shadow-2xl border border-amber-200/50 dark:border-amber-300/50 p-6 md:p-8 max-w-md w-full max-h-[90vh] overflow-y-auto pointer-events-auto"
          onClick={(e) => e.stopPropagation()}
        >
        {/* Close Button */}
        <button
          onClick={handleCancel}
          className="absolute top-4 right-4 p-2 rounded-full hover:bg-amber-50 dark:hover:bg-amber-200/50 transition-colors z-20"
          aria-label="Close"
        >
          <svg
        ...
        <p className="text-sm text-amber-700 dark:text-amber-800 mb-6">
          Help us maintain quality content. Select all reasons that apply.
        </p>
        ...
        <form onSubmit={handleSubmit}>
            ...
            {REPORT_REASONS.map((reason) => (
              ...
              >
                <input
                  ...
                  className="mt-1 w-4 h-4 text-amber-600 border-amber-300 rounded focus:ring-amber-500 focus:ring-2"
                />
          ...
          <div className="flex gap-3">
            ...
            <button
              type="submit"
              disabled={selectedReasons.length === 0 || (selectedReasons.includes('other') && !otherReason.trim())}
