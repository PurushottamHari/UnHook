    ...
    try:
        ...
        if categories:
            ...
            filtered_articles = fetch_articles(
                "default_user", "newest", category_filter=categories[0]
            )
            print(
                f"   Found {len(filtered_articles)} articles with category '{categories[0]}'"
            )

        # Test fetching articles with content type filter
        print("\n4. Testing fetch_articles() with content type filter 'MEDIUM'...")
        medium_articles = fetch_articles(
            "default_user", "newest", content_type_filter="MEDIUM"
        )
        ...
        long_articles = fetch_articles(
            "default_user", "newest", content_type_filter="LONG"
        )
        ...
        if categories:
            ...
            combined_articles = fetch_articles(
                "default_user",
                "newest",
                category_filter=categories[0],
                content_type_filter="MEDIUM",
            )
