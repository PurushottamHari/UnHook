...
async function getArticleDateForNavigation(article: CachedArticle): Promise<string | null> {
  try {
  ...
  return (
    <div className="min-h-screen bg-yellow-50 dark:bg-amber-50">
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="mb-8">
          <Link 
            href={navigationDate ? `/dashboard?date=${navigationDate}` : "/dashboard"}
        ...
        <article className="prose prose-lg max-w-none dark:prose-invert mb-12">
          <div className="bg-white/80 dark:bg-amber-100/80 backdrop-blur-sm rounded-xl shadow-lg border border-amber-200/50 dark:border-amber-300/50 p-8 md:p-12">
            <div className="prose prose-gray dark:prose-invert max-w-none">
              <ReactMarkdown 
                components={{
                h1: ({ children }) => <h1 className="text-2xl font-bold text-amber-900 dark:text-amber-900 mb-4">{children}</h1>,
                h2: ({ children }) => <h2 className="text-xl font-semibold text-amber-900 dark:text-amber-900 mb-3 mt-6">{children}</h2>,
                h3: ({ children }) => <h3 className="text-lg font-semibold text-amber-900 dark:text-amber-900 mb-2 mt-4">{children}</h3>,
                p: ({ children }) => <p className="text-amber-800 dark:text-amber-900 mb-4 leading-relaxed">{children}</p>,
                strong: ({ children }) => <strong className="font-semibold text-amber-900 dark:text-amber-900">{children}</strong>,
                em: ({ children }) => <em className="italic text-amber-800 dark:text-amber-900">{children}</em>,
                blockquote: ({ children }) => <blockquote className="border-l-4 border-amber-400 pl-4 italic text-amber-700 dark:text-amber-800 my-4">{children}</blockquote>,
                ul: ({ children }) => <ul className="list-disc list-inside mb-4 text-amber-800 dark:text-amber-900">{children}</ul>,
                ol: ({ children }) => <ol className="list-decimal list-inside mb-4 text-amber-800 dark:text-amber-900">{children}</ol>,
