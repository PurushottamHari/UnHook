...
import { cacheNewspaper, CachedNewspaper, CachedNewspaperArticle } from '@/lib/cache/newspaper-cache';
import { MongoClient } from 'mongodb';

async function fetchNewspaperFromMongoDB(date: string): Promise<CachedNewspaper | null> {
  try {
    ...
    if (!newspaper) {
      ...
      console.log(`Searching for created_at between: ${startOfDayTimestamp} and ${endOfDayTimestamp}`);
      console.log(`Date range: ${startOfDay.toISOString()} to ${endOfDay.toISOString()}`);
      
      // Debug: Show what newspapers are available
      const allNewspapers = await newspapersCollection.find({}).limit(5).toArray();
      console.log('Available newspapers:', allNewspapers.map(n => ({ 
        id: n._id, 
        created_at: n.created_at, 
        createdDate: n.created_at ? new Date(n.created_at * 1000).toISOString().split('T')[0] : 'No created_at field'
      })));
    ...
    for (const contentId of consideredContentIds) {
      const collectedDoc = await collectedContentCollection.findOne({ _id: contentId });
    ...
    const articles = await generatedContentCollection.find({
    ...
    const cachedArticles: CachedNewspaperArticle[] = articles.map(article => {
      ...
      const timeToRead = readingTimeSeconds > 0 ? `${Math.ceil(readingTimeSeconds / 60)}m read` : '5 min read';
