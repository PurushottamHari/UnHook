...
class ContentGenerator(BaseAIClient[ContentDataOutput]):
    ...
    async def generate_required_content(self, youtube_video_details : YouTubeVideoDetails, subtitle_map: SubtitleMap) -> dict[str, GeneratedData]:
        content_input = ContentInputAdaptor.from_youtube_details_and_subtitle_map(youtube_video_details, subtitle_map)
    ...
    def _create_prompt(self, content_input: ContentDataInput) -> str:
        input_data_no_subtitles = content_input.model_dump(exclude={"subtitle_data_input"})
