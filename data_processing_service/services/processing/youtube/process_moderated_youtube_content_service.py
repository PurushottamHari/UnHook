...
from data_processing_service.services.processing.youtube.subtitles.process_subtitles_for_youtube_content import ProcessSubtitlesForYoutubeContent
...
class ProcessModeratedYoutubeContentService:
    ...
    def process(self, user: User) -> None:
        ...
        """
        content_to_process_list = self.user_content_repository.get_user_collected_content(
            user_id=user.id,
        ...
        for content_to_process in content_to_process_list:
            try:
                ...
                subtitle_processed = self.subtitle_processor.process_subtitles(content_to_process)
                ...
                if subtitle_processed:
                    ...
                    self.logger.warning(f"Failed to process subtitles for content ID: {content_to_process.id}")
                ...
                self.logger.error(f"Error processing subtitles for content ID {content_to_process.id}: {str(e)}")
                continue
        ...
        if contents_with_subtitles_stored:
            ...
            self.user_content_repository.update_user_collected_content_batch(updated_user_collected_content_list=contents_with_subtitles_stored)    