...
import logging
from data_collector_service.models.user_collected_content import UserCollectedContent, ContentType
...
from data_collector_service.collectors.youtube.tools.youtube_external_tool import YouTubeExternalTool
from data_processing_service.services.processing.youtube.subtitles.utils.subtitle_utils import SubtitleUtils
...
class ProcessSubtitlesForYoutubeContent:
    ...
    def __init__(self):
        ...
        self.youtube_content_ephemeral_repository: YoutubeContentEphemeralRepository = LocalYoutubeContentEphemeralRepository()
    ...
    def process_subtitles(self, content: UserCollectedContent) -> bool:
        ...
        try:
            ...
            if self._is_subtitle_stored(video_details):
                self.logger.info(f"Subtitles already downloaded for content ID: {content.id}")
            else:
                # Download the subtitles
                if not self._download_and_store_subtitles(video_details):
                    self.logger.warning(f"Failed to download subtitles for content ID: {content.id}")
                    return False
            
            # Check if clean subtitles have already been stored, else continue
            if self._is_clean_subtitles_stored(video_details):
                self.logger.info(f"Clean subtitles already stored for content ID: {content.id}")
            else:
                # Clean the subtitles and store them in the ephemeral storage
                if not self._clean_and_store_subtitles(video_details):
                    self.logger.warning(f"Failed to clean and store subtitles for content ID: {content.id}")
                    return False
            
            return True
            
        except Exception as e:
            self.logger.error(f"Error processing subtitles for content ID {content.id}: {str(e)}")
            return False
    ...
    def _is_subtitle_stored(self, content: YouTubeVideoDetails) -> bool:
        ...
        """
        return self.youtube_content_ephemeral_repository.do_any_subtitles_exist_for_video(content.video_id)
    ...
    def _download_and_store_subtitles(self, content: YouTubeVideoDetails) -> bool:
        ...
        enriched_video_data_list = self.youtube_external_tool.enrich_video_data_with_details(youtube_video_details=[content])
                ...
                for ext in self.SUBTITLE_PRIORITY_LIST:
                    if subtitle_lang_data.get(ext):
                        try:
                            ...
                            subtitle_content = self.subtitle_utils.download_subtitle_file(url)
    ...
    def _is_clean_subtitles_stored(self, content: YouTubeVideoDetails) -> bool:
        ...
        """
        return self.youtube_content_ephemeral_repository.do_any_clean_subtitles_exist_for_video(content.video_id)
            ...
            for subtitle_type in ['automatic', 'manual']:
                for ext in self.SUBTITLE_PRIORITY_LIST:
                    if self.youtube_content_ephemeral_repository.check_if_downloaded_subtitle_file_exists(
                        ...
                        if subtitle_content:
                            cleaned_subtitles = self.subtitle_utils.clean_subtitles(subtitle_content, ext)
                            if cleaned_subtitles:
                                ...
                                self.logger.info(f"Successfully cleaned and stored {subtitle_type} subtitles with extension {ext} for video {content.video_id}")
        
        if not generated_clean_subtitles:
            self.logger.warning(f"Could not find any subtitles to clean for video {content.video_id}")
