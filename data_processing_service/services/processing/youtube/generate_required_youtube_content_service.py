...
from data_processing_service.repositories.user_content_repository import UserContentRepository
from data_processing_service.repositories.mongodb.user_content_repository import MongoDBUserContentRepository
...
from data_processing_service.repositories.ephemeral.local.youtube_content_ephemeral_repository import LocalYoutubeContentEphemeralRepository
from data_collector_service.models.user_collected_content import ContentType, ContentStatus, ContentSubStatus
import uuid
from datetime import datetime
from data_processing_service.models.generated_content import GeneratedContentStatus, StatusDetail
...
from data_processing_service.services.processing.youtube.ai_agent.content_generator import ContentGenerator
...
class GenerateRequiredYoutubeContentService:
    ...
    def __init__(self, user_content_repository: UserContentRepository, youtube_content_ephemeral_repository: YoutubeContentEphemeralRepository):
        """
    ...
    async def generate(self, user_id: str) -> None:
        ...
        for content_to_generate in content_to_generate_list:
            try:
                ...
                youtube_video_details = content_to_generate.data.get(ContentType.YOUTUBE_VIDEO)
                subtitle_data = self.youtube_content_ephemeral_repository.get_all_clean_subtitle_file_data(video_id=content_to_generate.external_id)
                selected_subtitle = self._select_best_subtitle(subtitle_data, youtube_video_details)
                generated_data = await self.content_generator_agent.generate_required_content(youtube_video_details=youtube_video_details, subtitle_map=selected_subtitle)
                ...
                self.user_content_repository.add_generated_content(generated_content=generated_content)
                   
            except Exception as e:
                self.logger.error(f"Content generation failed for {content_to_generate.external_id}: {e}")
                continue

    def _select_best_subtitle(self, subtitle_data: SubtitleData, youtube_video_details: YouTubeVideoDetails) -> SubtitleMap:
        """
        ...
        for sub in subtitle_data.manual:
            if sub.language.lower() == preferred_language.lower() and sub.subtitle.strip():
                return sub
...
if __name__ == "__main__":
    ...
    service = GenerateRequiredYoutubeContentService(user_content_repository, youtube_content_ephemeral_repository)
