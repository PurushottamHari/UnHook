...
from .models import ModerationInput, ModerationOutput, ContentItem
import json
...
class ContentModerator(BaseAIClient[ModerationOutput]):
    ...
    async def moderate_content(
        ...
        prompt = self._create_prompt(moderation_input.items, moderation_input.filter_preferences)
        return await self.generate_structured_response(user_input=prompt)
    
    def _create_prompt(
        self,
        items: List[ContentItem],
        filter_preferences: List[str]
    ) -> str:
