...
def extract_article_id(input_str: str) -> Optional[str]:
    ...
    if input_str.startswith("http://") or input_str.startswith("https://"):
        ...
        if re.match(f"^{uuid_pattern}$", input_str, re.IGNORECASE):
            ...
            print(
                f"Warning: Input does not appear to be a valid article ID: {input_str}"
            )
...
class ArticleProcessor:
            ...
            "category": (
                {
                    ...
                    "shelf_life": (
                        article.category.shelf_life if article.category else None
                    ),
                    "geography": (
                        article.category.geography if article.category else None
                    ),
                }
                if article.category
                else None
    ...
    def add_newspaper_to_date_folder(
        self, newspaper: Dict[str, Any], date_folder: Path
    ) -> None:
    ...
    def create_social_media_file(
        self, article: GeneratedContent, today_folder: Path
    ) -> None:
        ...
        if len(short_content) > max_summary_length:
            short_content = (
                short_content[:max_summary_length].rsplit(" ", 1)[0] + "....."
            )
...
"""
    ...
    def generate_social_media_card(
        self, article: GeneratedContent, today_folder: Path
    ) -> None:
        ...
        try:
            ...
            if category_text:
                ...
                draw.rounded_rectangle(
                    [
                        (cat_pill_x, cat_pill_y),
                        (cat_pill_x + cat_pill_width, cat_pill_y + cat_pill_height),
                    ],
                    radius=16,
                ...
                cat_text_y = (
                    cat_pill_y + (cat_pill_height - 18) // 2
                )  # Center vertically
                draw.text(
                    (cat_text_x, cat_text_y),
                    category_text,
                    fill=category_text_color,
                    font=tiny_font,
                )
            ...
            if logo_path.exists():
                try:
                    ...
                    logo_y = (
                        card_height - logo_height - padding - 10
                    )  # Closer to bottom
    ...
    def start_local_marketing_frontend(self) -> bool:
        ...
        try:
            ...
            if not self.marketing_frontend_dir.exists():
                print(
                    f"❌ Marketing frontend directory not found: {self.marketing_frontend_dir}"
                )
                return False
            ...
            while waited < max_wait_time:
                try:
                    ...
                    response = requests.get(
                        f"http://localhost:{self.local_app_port}", timeout=5
                    )
                    if response.status_code in [
                        200,
                        404,
                    ]:  # 404 is fine, means app is running
                        print(
                            f"✅ Local marketing frontend started successfully on port {self.local_app_port}"
                        )
                        return True
                ...
                if self.local_app_process.poll() is not None:
                    ...
                    print(
                        f"❌ Local app failed to start. stdout: {stdout}, stderr: {stderr}"
                    )
                    return False
    ...
    def make_api_calls(self, article_id: str) -> None:
        ...
        try:
            ...
            if local_app_started:
                ...
                try:
                    local_article_response = requests.post(
                        local_article_url, timeout=30
                    )
                    print(
                        f"Local Article API response: {local_article_response.status_code}"
                    )
                    ...
                    if local_article_response.status_code == 200:
                        ...
                        print(
                            f"⚠️ Unexpected local response: {local_article_response.status_code}"
                        )
                        if local_article_response.text:
                            print(
                                f"Local response body: {local_article_response.text[:200]}..."
                            )
                except Exception as e:
            ...
            if local_app_started:
                ...
                try:
                    local_newspaper_response = requests.get(
                        local_newspaper_url, timeout=30
                    )
                    print(
                        f"Local Newspaper API response: {local_newspaper_response.status_code}"
                    )
                    ...
                    if local_newspaper_response.status_code == 200:
                        ...
                        print(
                            f"⚠️ Unexpected local response: {local_newspaper_response.status_code}"
                        )
                        if local_newspaper_response.text:
                            print(
                                f"Local response body: {local_newspaper_response.text[:200]}..."
                            )
                except Exception as e:
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python article_processor.py <article_id_or_url1> [article_id_or_url2] ... [--test]"
        )
        print("  --test: Generate only the image without running full script")
        print(
            "  You can provide multiple article IDs or URLs to process them sequentially"
        )
        print("  Examples:")
        print("    python article_processor.py 79bc5706-23e0-4fad-b797-ec0467f7e713")
        print(
            "    python article_processor.py https://unhook-production.up.railway.app/article/79bc5706-23e0-4fad-b797-ec0467f7e713"
        )
        sys.exit(1)
    ...
    try:
        ...
        for i, article_id in enumerate(article_ids, 1):
            ...
            try:
                ...
                print(
                    f"✅ Successfully processed article {i}/{len(article_ids)}: {article_id}"
                )
            except Exception as e:
                print(
                    f"❌ Error processing article {i}/{len(article_ids)} ({article_id}): {e}"
                )
