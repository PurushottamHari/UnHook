...
from ...repositories.user_collected_content_repository import UserCollectedContentRepository
...
class YouTubeStaticCollector(BaseStaticCollector):
    ...
    def collect(self, user: User) -> None:
        ...
        for channel in channels:
            ...
            latest_videos = self.youtube_tool.find_latest_videos(channel_name=channel_name, max=max_videos)
            ...
            user_id = str(user.id)
            uncollected_video_ids = self.user_repository.filter_already_collected_content(user_id, latest_video_ids)
            
            uncollected_videos = [video for video in latest_videos if video.video_id in uncollected_video_ids]
            ...
            enriched_uncollected_videos = self.youtube_tool.enrich_video_data_with_details(youtube_video_details=uncollected_videos)

            # Step 3: Convert videos to user collected content and add to user's history
            user_collected_content = self.adapter.convert_batch(enriched_uncollected_videos, user_id)
