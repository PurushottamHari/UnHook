...
from data_collector_service.collectors.youtube.tools.exceptions.youtube_video_key_data_missing import youtube_video_key_data_missing
from data_collector_service.collectors.youtube.tools.adapters.subtitle_adapter import SubtitleAdapter
...
class YouTubeVideoAdapter:
    ...
    def to_video_details(video_data: Dict, channel_name: str) -> YouTubeVideoDetails:
        ...
        if not video_data.get('id') or not video_data.get('title'):
            raise youtube_video_key_data_missing("'id' and 'title' are required fields and must not be empty.")
        ...
        if thumbnails:
            ...
            highest_res = max(thumbnails, key=lambda x: x.get('width', 0) * x.get('height', 0))
    ...
    @staticmethod
    def to_video_details_list(videos_data: List[Dict], channel_name: str) -> List[YouTubeVideoDetails]:
        """
        ...
        for video in videos_data:
            try:
                ...
                logger.error(f"Missing key data for video in channel {channel_name}: {str(e)}")
            except Exception as e:
                logger.error(f"Unexpected error processing video in channel {channel_name}: {str(e)}")
        return result 
    
    @staticmethod
    def enrich_video_detail(old_youtube_video_details: YouTubeVideoDetails, enrich_details_dict: Dict) -> YouTubeVideoDetails:
        """ 
        ...
        if not release_epoch:
            raise youtube_video_key_data_missing("'timestamp/release_timestamp' is a required field for enrichment: " + old_youtube_video_details.video_id)
        subtitles = SubtitleAdapter.from_enrich_dict(enrich_details_dict)
        if not subtitles:
            logger.warn("Subtitles not found for: " + old_youtube_video_details.video_id)
            return None
        ...
        new_video_details.categories = enrich_details_dict.get('categories', new_video_details.categories)
        new_video_details.language = enrich_details_dict.get('language', new_video_details.language)
        new_video_details.release_date = datetime.fromtimestamp(release_epoch)
        new_video_details.title = enrich_details_dict.get('title', new_video_details.title)
        new_video_details.description = enrich_details_dict.get('description', new_video_details.description)
        new_video_details.views = enrich_details_dict.get('view_count', new_video_details.views)
        new_video_details.likes_count = enrich_details_dict.get('like_count', new_video_details.likes_count)
        new_video_details.comments_count = enrich_details_dict.get('comment_count', new_video_details.comments_count)
        new_video_details.duration_in_seconds = enrich_details_dict.get('duration', new_video_details.duration_in_seconds)
