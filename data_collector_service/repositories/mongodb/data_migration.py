...
from data_collector_service.repositories.mongodb.adapters.youtube_video_details_adapter import YouTubeVideoDetailsAdapter
...
def migrate_timestamps():
    ...
    print(f"Connected to database '{settings.DATABASE_NAME}' and collection '{settings.COLLECTION_NAME}'.")
    ...
    for doc in documents:
        ...
        if 'created_at' in doc and isinstance(doc['created_at'], str):
            try:
                ...
                print(f"Could not parse created_at for doc {doc_id}: {doc['created_at']}")
                continue

        # Check and update updated_at
        if 'updated_at' in doc and isinstance(doc['updated_at'], str):
            try:
                dt_obj = CollectedContentAdapter._convert_iso_to_datetime(doc['updated_at'])
                updates['updated_at'] = dt_obj.timestamp()
            except ValueError:
                print(f"Could not parse updated_at for doc {doc_id}: {doc['updated_at']}")
                continue
...
def migrate_data_structure():
    ...
    for doc in documents:
        ...
        if data and isinstance(data, dict) and 'YOUTUBE_VIDEO' not in data and 'video_id' in data:
            try:
                video_data = data
                ...
                video_details_payload = {
                    ...
                    'duration_in_seconds': safe_int(video_data.get('duration_in_seconds')),
                ...
                new_data_field = {
                    "YOUTUBE_VIDEO": video_details.dict(by_alias=True, exclude_none=True)
                }
...
def migrate_status_details_timestamps():
    ...
    for doc in documents:
        ...
        for detail in status_details:
            if isinstance(detail, dict) and 'created_at' in detail and isinstance(detail['created_at'], str):
                try:
                    ...
                    print(f"Could not parse created_at for doc {doc_id} in status_details: {detail['created_at']}. Error: {e}")
            new_status_details.append(detail)
        
        if made_change:
            collection.update_one({'_id': doc_id}, {'$set': {'status_details': new_status_details}})
            updated_count += 1
            print(f"Updated document {doc_id}")
...
def migrate_status_value():
    ...
    query = {
        "$or": [
            {"status": "PROCESSED"},
            {"status_details.status": "PROCESSED"}
        ]
    }
...
def migrate_add_moderation_passed_sub_status():
    ...
    for doc in documents:
        ...
        if not processing_status_detail or 'created_at' not in processing_status_detail:
            print(f"Skipping doc {doc_id}: No 'PROCESSING' status detail with a timestamp found.")
            continue
...
def migrate_enrich_subtitles():
    ...
    for doc in documents:
        ...
        try:
            ...
            video_details_entity = YouTubeVideoDetailsAdapter.from_db_model(video_details_db)
            
            enriched_videos = youtube_tool.enrich_video_data_with_details(youtube_video_details=[video_details_entity])

            if enriched_videos and enriched_videos[0].subtitles:
                enriched_video_db = YouTubeVideoDetailsAdapter.to_db_model(enriched_videos[0])
...
def migrate_delete_subtitles():
    ...
    query = {
        "data.YOUTUBE_VIDEO.subtitles": {"$exists": True}
    }
    update_operation = {
        "$unset": {"data.YOUTUBE_VIDEO.subtitles": ""}
    }
    
    result = collection.update_many(query, update_operation)
    
    print(f"Subtitle deletion migration complete. {result.modified_count} documents updated.")
