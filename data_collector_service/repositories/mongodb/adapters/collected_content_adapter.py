...
from ..models.collected_content_db_model import CollectedContentDBModel, SubStatusDetails, StatusDetails
from data_collector_service.models.user_collected_content import UserCollectedContent, StatusDetail, ContentStatus, ContentType, SubStatusDetail
...
class CollectedContentAdapter:
    ...
    @staticmethod
    def to_collected_content_db_model(content: UserCollectedContent) -> CollectedContentDBModel:
        ...
        status_details = [
            StatusDetails(
                ...
                created_at=CollectedContentAdapter._datetime_to_float(detail.created_at)
        ...
        if content.content_type == ContentType.YOUTUBE_VIDEO:
            ...
            if isinstance(video_details, YouTubeVideoDetails):
                data_db_model[ContentType.YOUTUBE_VIDEO] = YouTubeVideoDetailsAdapter.to_db_model(video_details)
            else:
                raise TypeError(f"Expected YouTubeVideoDetails, but got {type(video_details)}")
        else:
    ...
    @staticmethod
    def to_user_collected_content(db_model: CollectedContentDBModel) -> UserCollectedContent:
        """Convert CollectedContentDBModel to UserCollectedContent."""
        status_details = [
            StatusDetail(
                status=ContentStatus(detail.status),
                created_at=CollectedContentAdapter._float_to_datetime(detail.created_at),
                reason=detail.reason
        ...
        if hasattr(db_model, 'sub_status_details') and db_model.sub_status_details:
            for detail in db_model.sub_status_details:
                sub_status_details.append(
                    SubStatusDetail(
                        sub_status==detail.sub_status,  # Reuse StatusDetail for sub_status
                        created_at=CollectedContentAdapter._float_to_datetime(detail.created_at),
                        reason=detail.reason
                    )
                )
